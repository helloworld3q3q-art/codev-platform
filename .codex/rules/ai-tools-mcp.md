...
>
> **Codex 口径**：本文件位于 `.codex/rules/`，Codex 通过 streamable HTTP `/mcp` 连接平台服务。`.claude/**`、SSE/stdio 说明仅作为旧 Claude Code 兼容背景。
>
> 当前四个项目 MCP：
> - `platform-docs`: `http://127.0.0.1:18083/mcp?project_id=codev-platform`
> - `codegraph`: `http://127.0.0.1:18091/mcp?project_id=codev-platform`
> - `agent-memory`: `http://127.0.0.1:18087/mcp?project_id=codev-platform`
> - `graph`: `http://127.0.0.1:18092/mcp?project_id=codev-platform`
>
> 认证统一走 `.codex/config.toml` / 用户级 `~/.codex/config.toml` 的 `bearer_token_env_var = "CODEV_PLATFORM_MCP_TOKEN"`。修改 token 或 MCP 配置后需要重启 Codex 会话，并用 `codex mcp list` / `codex mcp get <name>` 验证。健康检查不能只看 `/health`，至少实调一个 MCP 工具。
...
## 一 b、MCP 接入方式 —— Codex streamable HTTP /mcp

Codex 当前使用 `.codex/config.toml` 中的 streamable HTTP MCP 配置，端点统一为 `/mcp`，端口为 18083 / 18091 / 18087 / 18092。Claude Code 旧 `.mcp.json` 的 SSE/stdio 配置只作为兼容背景，不是 Codex 主入口。

本仓 Windows 本地开发以 180xx 为当前真值入口；若临时接 WSL relay 的 190xx，必须确保代码、Chroma 数据和 codegraph 索引也在同一套环境重建，避免 MCP 召回旧仓库。

验证顺序：

```powershell
codex mcp list
codex mcp get platform-docs
codex mcp get codegraph
```

判活必须实调 MCP 工具；只看进程、端口或 `/health` 不够。

## 一 c、历史背景：平台 SSE 服务地址(2026-05-30 服务化)
...
子模块 `.codex/rules/`(约 2200 行)**不会自动加载**。改子模块代码**前**必须先:
...
| **想退回旧 stdio 文件路径模式** | 仅适用于 Claude 兼容入口：`copy <业务仓>\.mcp.json.stdio.bak <业务仓>\.mcp.json` → 重启 Claude Code,恢复 per-session 自 spawn(单机略快,无需 serve-mcp start) |
...
| platform-docs MCP `-32602 Invalid params` 持续 | Codex 侧先重启 Codex 会话并检查 `.codex/config.toml`；Claude 兼容入口再检查 `.cmd` 行尾 CRLF |
...
| platform-docs **daemon 运行中崩了** (search_docs 突然全失败) | mcp-proxy 还连着死的 daemon,所有 search 报错。**重启 Codex 会话**让 MCP 连接重建；Claude 兼容入口再重启 Claude Code。`ai-health` 的 `platform-docs daemon` 会报 `not running`,`platform-docs servers` 会报 `0 servers`,组合判断 = daemon 真死了。常见诱因:GPU 驱动更新 / Windows 系统重启 / 手动 kill |
...
| MCP 完全连不上 | Codex 侧检查 `.codex/config.toml` / `~/.codex/config.toml`、`CODEV_PLATFORM_MCP_TOKEN` 和 `codex mcp list/get`；Claude 兼容入口再检查 `~/.claude.json` |
...
【文件路径】 .codex\rules\cross-layer-enum-consistency.md
【背景关键词】 N12 / FETCHER_GROUP_MAP / 单一真值源
【禁止】 不超过 150 行 / 不在 AGENTS.md 加 @ 引用 / 不抄旧规则原文
