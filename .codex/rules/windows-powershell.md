...
> Codex 口径：本规则中的旧 `Edit / Write` 可理解为 `apply_patch / 文件编辑`，旧 `Bash` 可理解为 `shell_command`。`.claude/**` 相关 hook 或工具名只作为迁移期兼容背景；实际执行以 Codex 工具和仓根 `AGENTS.md` 为准。
...
## 6. 旧 Claude Code 兼容背景：安全检查友好的命令写法

Claude Code CLI 对部分 PowerShell / Bash 模式有**硬编码安全检查**，触发后会跳出确认弹窗，且 `.claude/settings.local.json` 的 allow 规则**无法覆盖**。Codex 当前不使用 Claude 的 `Monitor` / hook 机制；本节作为迁移期兼容背景保留，实际执行以 Codex 可用工具和本文件前文规则为准。
...
| 场景 | 不要这么做 | 改用 |
|---|---|---|
| 轮询等待后台任务（Java 启动 / 回测进度） | `for($i=0; ...) { Start-Sleep; Test-NetConnection ... }` 加字符串插值打印进度 | Codex 用可用的进程/日志检查方式；旧 Claude 可用 `Monitor` |
| 读后台任务 stdout | `Get-Content "...\tasks\xxxx.output" -Tail N` 加 `(pipeline).Count` | Codex 直接读取目标日志或等待命令完成；旧 Claude 可用 `Monitor` |
...
- 等任务 → Codex 用进程/日志检查；旧 Claude 可用 `Monitor`
