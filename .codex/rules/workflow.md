...
- 运行态 / 索引产物：`data/`、`.codegraph/`、`<PLATFORM_DATA_DIR>/mcp_serve_logs/`
...
- 凡会修改仓库文件、执行部署、改变运行态、提交或推送的独立任务，必须先在
  `docs/plans/roadmap-YYYY-MM-DD/` 创建计划；**计划文件是任务首个允许写入的文件**。
- 计划落盘后，第二步在同目录 `README.md` 登记状态；计划和登记完成前，不得修改实现、规则、
  测试、运行态或 Git 状态。本规则变更本身也不例外。
- 同一目标下用户说“继续”、复审返修或补验证时，更新原计划的状态与证据，不重复创建新计划；
  目标、范围或交付物发生独立变化时才新建计划。
- 纯只读问答、状态查询和诊断不创建空计划；一旦诊断转为修复或任何外部写操作，必须先建计划。
- plan、design、task brief、progress、completion report 的任务真值统一放入上述 roadmap 目录。
  `.superpowers/**` 只允许非真值的临时测试证据或工具输出，禁止新增或更新任务文档；
  `docs/superpowers/**` 也不是本仓计划落点。任何 skill 声明的默认保存路径均由本规则覆盖。
- 计划至少写明目标、范围、适用规则、关键约束、有序步骤、验证命令和当前状态；不能用聊天中的
  临时清单代替仓库计划。
...
- 日常 `commit` 并仅推送 `origin/dev` 后，默认只由既有 post-commit hook / WSL 串行 worker 消费已入队 scope，增量更新
  Chroma、CodeGraph、graph ingest、code_vec 与相应 manifest；Git push 不等同数据库已完成，worker 停止或 maintenance 时允许
  pending 保持，恢复既有状态机后继续消费。
- 禁止因为 pending、索引滞后、MCP 返回旧提交、服务暂停或一次配置诊断而执行 runtime/base/release/wheel 重建、依赖下载、
  全量 reindex、数据库重置/重建、重新部署或创建第二套运行环境。
- 仅以下三种情形可申请重建：用户本轮明确要求；已批准的项目迁移/结构性重构；或已有可复核证据证明数据库/索引损坏且增量恢复
  不可行。例外动作前必须先更新 `docs/plans/roadmap-YYYY-MM-DD/` 的现有计划和同目录 README，说明证据、最小范围、回退与
  为什么普通增量队列不足；不得以“更快”或“省排查”为理由。
