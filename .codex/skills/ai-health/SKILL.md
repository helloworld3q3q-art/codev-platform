...
> Codex 口径：若当前会话没有交互式多选能力，按用户原话选择最小模式；不确定时先跑 `codev-platform health`，需要索引一致性再跑 `codev-platform dirty-check`。旧交互式提问说明仅作兼容背景。
...
**当用户只说"AI 工具状态"/"检查工具栈"不指明具体诉求时,Codex 按最小可用范围执行；不确定时默认 `both`，并在执行前简短说明**:
...
**注意**:exit 2 是 WARN 不是失败,部分运行器会标"Error"但工具栈正常,看 SUMMARY 行为准。
...
- 触发指南:`.codex/rules/ai-tools-mcp.md`(CodeGraph / Chroma / graph 何时用)
