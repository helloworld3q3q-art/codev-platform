...
1. **任务先建计划**：每个会修改文件或外部状态的独立任务，首个允许写入必须是 `docs/plans/roadmap-YYYY-MM-DD/` 中的计划；禁止把 plan/task 真值写入 `.superpowers/**` 或 `docs/superpowers/**`。
2. **改文件前**：先读 `.codex/rules/workflow.md` 的任务分级和任务映射，按本次任务读取对应规则。
3. **先说明门禁**：首次修改前说清楚触及层、适用规则、关键约束、验证方式。
4. **不回滚用户改动**：发现未知 dirty 文件时，先判断是否相关；相关且冲突再问。
5. **MCP-first**：查代码结构先用 `codegraph`，查规则/设计/事故先用 `platform-docs`，查跨层影响先用 `graph`，跨会话记忆用 `agent-memory`。
6. **本地搜索是兜底**：只有未提交改动、索引滞后、MCP 不可用、需要精确核对真实文件时，才用本地搜索/读取兜底，并说明原因。
7. **禁改默认范围**：`.venv/`、`data/`、`.codegraph/`、`node_modules/`、构建产物、用户级 `~/.codev-platform/config.json` 默认不手改。
8. **生成物纪律**：`web-ui/src/services/**` 优先由 `pnpm run api` 生成；确需手改先说明原因。
9. **安全纪律**：不输出明文 token/API key/DSN；commit message 不带 AI 痕迹。
10. **PowerShell 编码**：读中文 MD/JSON/log 使用 `-Encoding UTF8`；`.ps1` 保持 ASCII。
11. **Claude 兼容层**：不要删除 `.claude/**` 或 `CLAUDE.md`；它们不是新的维护入口。
