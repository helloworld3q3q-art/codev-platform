...
class RemoteEmbedder(Embedder):
    def __init__(self, url: str, *, timeout: float = 30.0, token: str | None = None,
                 internal_call: str | None = None, batch_size: int = _EMBED_HTTP_BATCH) -> None:
        self._url = url
        if isinstance(timeout, bool) or not math.isfinite(float(timeout)) or float(timeout) <= 0:
            raise ValueError("remote embed timeout 必须是有限正数")
        if isinstance(batch_size, bool) or type(batch_size) is not int or batch_size <= 0:
            raise ValueError("remote embed batch_size 必须是正整数")
    ...
    def encode_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        子批大小由实例策略收口，不做 N 次单条往返，也不长时间独占推理服务。"""
        out: list[list[float]] = []
    ...
    def _post_embed_retry(self, payload: dict, *, expect: int) -> list[list[float]]:
        ...
        for attempt in range(_EMBED_RETRIES):
            try:
                data = _post_json(self._url, payload, self._timeout, self._token, self._internal_call)
                vecs = data.get("vectors")
