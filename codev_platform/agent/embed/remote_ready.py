"""code_vec 远程嵌入服务的就绪门禁。

通用远程 embedder 保持无副作用；只有 code_vec 写路径在昂贵构建前证明
platform-docs 已就绪。隔离 attempt 不得自行创建常驻服务。
"""
...
_ISOLATED_ATTEMPT_ENV = "CODEV_REINDEX_CGROUP_ROOT"
...
def _inside_isolated_attempt() -> bool:
    """只有 cgroup bootstrap 的后代才携带该环境，避免把普通 CLI 误判为隔离执行。"""
    return bool(os.environ.get(_ISOLATED_ATTEMPT_ENV))
...
def ensure_code_vec_remote_daemon(cfg: dict) -> dict[str, Any]:
    ...
    if _inside_isolated_attempt():
        raise RuntimeError(
            "code_vec 隔离 attempt 中 platform-docs 未就绪；"
            "为保证 containment 可收口，不得自启常驻 daemon，请先启动 systemd 服务"
        )
