...
_ctx_project: contextvars.ContextVar[str | None] = contextvars.ContextVar(
    "_mem_project", default=None
)
...
def _entry_dict(e) -> dict:
    ...
    return {
        ...
        "is_redline": e.is_redline,
        "status": e.status,
        "task_id": e.task_id,
        "task_state": e.task_state,
    }
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "query": {
                        "type": "string",
                        "description": "检索词(可空;空则按 redline>recency 返回)",
                    },
                    "limit": {"type": "integer", "default": 8, "minimum": 1, "maximum": 50},
                    "task_id": {
                        "type": "string",
                        "description": "可选:当前任务 id,匹配的任务记忆前置",
                    },
                    "policy": {
                        "type": "string",
                        "enum": list(_POLICIES),
    ...
    try:
        ...
        if name in ("remember", "supersede"):
            ...
            entry = MemoryEntry(
                ...
                content=content,
                kind=args.get("kind"),
                topic_key=make_topic_key(args.get("topic_key")),
                is_redline=False,
            )
            if name == "remember":
        ...
        if name == "forget":
            ...
            # owner 限定即鉴权:只能删本人 + 本 org 的非 redline 条目(IDE 不得删 org 硬约束)。
                ...
                entry_id, owner_user_id=user_id, org_id=org_id, protect_redline=True
            )
            return _ok(
                {
                    "ok": done,
...
async def run_http(port: int = _MEM_SSE_PORT) -> None:
    require_managed_mcp_service("agent_memory")
    ...
    def bind_mcp_context(request):
        ...
        if pid_raw:
            ...
            if not dec.allowed:
                _flog(
                    f"[mcp] DENY project_id={pid} via={getattr(ident, 'via', None)}: {dec.reason}"
                )
                return JSONResponse(
    ...
    async def handle_sse(request):
        ...
        if pid_raw:
            ...
            if not dec.allowed:
                _flog(
                    f"[sse] DENY project_id={pid} via={getattr(ident, 'via', None)}: {dec.reason}"
                )
                return JSONResponse(
