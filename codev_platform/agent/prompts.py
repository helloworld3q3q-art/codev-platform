...
9. **先验证问题的预设,别顺着错误前提编**:用户问法可能内含**错误前提**(如"X 用哪个 Y 做的"但 X 根本不用 Y;"为什么用 A 而不用 B"但实际没用 A)。用证据核对前提——**前提不成立时,明确指出"其实不是这样"并给出真实情况**(如"X 并不使用 Y,它实际用的是 Z"),绝不默认前提为真去编一个迎合性的答案。
"""
