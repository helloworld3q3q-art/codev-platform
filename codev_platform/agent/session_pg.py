...
依赖 psycopg[binary,pool](在 [agent] extra)。运行期只读验证 Alembic 已迁移 schema。
"""
...
_SCHEMA_PROBES = (
    "SELECT org_id, user_id, session_id, project_id, created_at, updated_at "
    "FROM agent_sessions LIMIT 0",
    "SELECT id, org_id, user_id, session_id, role, content, payload, created_at "
    "FROM agent_messages LIMIT 0",
)
