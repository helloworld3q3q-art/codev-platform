...
_PROVEN_INDEX_CONFIG_MAX_BYTES = 256 * 1024
...
def _active_index_config() -> Path | None:
    return next((path for path in _index_config_candidates() if path.is_file()), None)


def _read_project_index_config(
    cfg: Path | None,
    *,
    strict: bool = False,
    ...
    try:
        if strict and cfg.stat().st_size > _PROVEN_INDEX_CONFIG_MAX_BYTES:
            raise ValueError("受证明 index.json 超过大小上限")
        data = json.loads(cfg.read_text(encoding="utf-8"))
        if strict and not isinstance(data, dict):
            raise ValueError("受证明 index.json 根必须是对象")
        if isinstance(data.get("doc_patterns"), list) and data["doc_patterns"]:
            if strict and any(type(item) is not str for item in data["doc_patterns"]):
                raise ValueError("受证明 doc_patterns 只能包含字符串")
        ...
        if isinstance(data.get("external_doc_paths"), list):
            if strict and any(type(item) is not str for item in data["external_doc_paths"]):
                raise ValueError("受证明 external_doc_paths 只能包含字符串")
            external = [str(p) for p in data["external_doc_paths"]]
            if external:
                logger.info("loaded %d external_doc_paths from %s", len(external), cfg)
    except (json.JSONDecodeError, OSError) as exc:
        if strict:
            raise ValueError("受证明 index.json 无法解析") from exc
        logger.warning("failed to parse %s: %s, fallback to default", cfg, exc)
    return patterns, external


def _load_project_index_config() -> tuple[list[str], list[str]]:
    """兼容入口；发现路径使用同一个已选配置，避免两次查找产生漂移。"""
    return _read_project_index_config(_active_index_config())
...
def discover_files() -> list[Path]:
    ...
    config_path = _active_index_config()
    if guard is not None and config_path is not None:
        guard.validate_files((config_path,))
    patterns, external = _read_project_index_config(
        config_path,
        strict=guard is not None,
    )
    if guard is not None:
        seen = {
            path
            for path in guard.match_main_files(patterns)
            if not is_excluded(path.relative_to(PLATFORM_ROOT))
        }
        seen.update(guard.match_external_files(PLATFORM_ROOT, external))
        return sorted(seen)
    seen: set[Path] = set()
