...
from codev_platform.chroma._config import (
    DATA_DIR, EMBED_DEVICE, EMBED_MAX_SEQ_LENGTH, EMBED_MAX_SEQ_LENGTH_OVERRIDE,
    EMBED_MODEL, GPU_CONCURRENCY,
)
    ...
    try:
        ...
        try:
            ...
            if EMBED_DEVICE != "cpu" and _is_gpu_error(gpu_exc):
                ...
                raise
        effective_limit = (
            resolve_embed_max_seq_length(actual_device, EMBED_MAX_SEQ_LENGTH_OVERRIDE)
            if actual_device != EMBED_DEVICE
            else EMBED_MAX_SEQ_LENGTH
        )
        apply_embed_max_seq_length(_model, effective_limit)
