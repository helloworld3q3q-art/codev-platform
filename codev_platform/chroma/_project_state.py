...
from codev_platform.core.paths import chroma_collection_name, chroma_docs_dir
...
def _project_last_indexed_iso(project_id: str) -> str | None:
    ...
    # per-project stamp: 每项目独立库 chroma_docs_dir(pid)/.last_build.json (indexer 写)
    pp = chroma_docs_dir(project_id) / ".last_build.json"
    if pp.exists():
        try:
...
def _maybe_reload_project(state: _ProjectState) -> bool:
    ...
    不再使其它项目误 reload。"""
    pp = chroma_docs_dir(state.project_id) / ".last_build.json"
