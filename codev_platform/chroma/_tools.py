...
from codev_platform.chroma._embed_api import _gpu_call
from codev_platform.chroma._schema import _err, _ok, _filter_miss_content, _build_where
    ...
    try:
        if name == "search_docs":
            ...
            async with _get_gpu_sem():
                qvec = await _gpu_call(lambda: _encode_query(query))
            ...
            for idx, _id in enumerate(ids):
                ...
                out.append(item)
            filter_miss = not out and where is not None
            ...
            if filter_miss:
                recall_event["diagnostic"] = "filter_miss"
            _log_recall(recall_event)

            content = _ok(out)
            if filter_miss:
                diagnostic = _filter_miss_content(category, module)
