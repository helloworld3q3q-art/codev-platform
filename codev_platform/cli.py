...
# platform_meta 注册表路径: 显式环境覆盖 > 随 wheel 发布的只读资源。
# 保留模块常量，兼容现有 CLI / Web 测试的定向替换。
PLATFORM_META_PROJECTS = platform_meta_projects_dir()
    ...
    if args.action == "ingest":
        ...
        with maintenance_reindex_operation_permit() as permitted:
            if permitted is not True:
                _eprint("FATAL: reindex 维护窗口已启用；graph ingest 写入被拒绝")
                return 1
...
def cmd_web(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        return 1
    target = "codev_platform.web.app:create_runtime_app"
    _print(f"启动 web backend: {target} @ {args.host}:{args.port}")
    uvicorn.run(
        target,
        host=args.host,
        port=args.port,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sp_sr = sub.add_parser("sync-rules", help="把平台规则按需同步到 Codex/Claude 目录")
    sp_sr.add_argument("--dry-run", action="store_true", help="只列不写")
    sp_sr.add_argument(
        "--target",
        choices=("claude", "codex", "both"),
        default="claude",
        help="目标入口（codex 仅同步已声明 surface 兼容的规则）",
    )
    sp_sr.add_argument(
        "--only",
        action="append",
        default=None,
        metavar="NAME",
        help="仅同步指定文件/顶层目录，可重复",
    )
    sp_sr.set_defaults(func=cmd_sync_rules)

    sp_ss = sub.add_parser("sync-skills", help="把平台 skills 按需同步到 Codex/Claude 目录")
    sp_ss.add_argument("--dry-run", action="store_true", help="只列不写")
    sp_ss.add_argument(
        "--target",
        choices=("claude", "codex", "both"),
        default="claude",
        help="目标入口（codex 仅同步已声明 surface 兼容的 skills）",
    )
    sp_ss.add_argument(
        "--only",
        action="append",
    ...
    sp_mcp.add_argument("--config-user", default=None,
                        help="install-systemd: 仅 Linux root 按该服务账号 HOME 解析配置，须与 --user 一致")
    sp_mcp.add_argument(
        "--runtime-root",
        type=Path,
        default=None,
        help="install-systemd: 版本化运行时根目录",
    )
    sp_mcp.add_argument(
        "--no-restart",
        action="store_true",
        help="install-systemd: 仅预装 unit，不改变服务运行状态",
    )
