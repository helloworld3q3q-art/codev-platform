...
def data_root(
    *,
    ...
    """
    selected = _configured_data_value(cfg=cfg, environment=environment)
    if selected is not None:
        value, field = selected
        return _resolved_data_path(value, field)
    return _business_repo_root() / "data"


def privileged_data_root(
    *,
    cfg: dict[str, Any] | None = None,
    environment: Mapping[str, str] | None = None,
) -> Path:
    """返回供特权文件操作使用的显式词法数据根，不展开用户目录或符号链接。"""
    selected = _configured_data_value(cfg=cfg, environment=environment)
    if selected is None:
        raise ValueError("特权数据根必须显式配置")
    value, field = selected
    return _lexical_data_path(value, field)


def _configured_data_value(
    *,
    cfg: dict[str, Any] | None,
    environment: Mapping[str, str] | None,
) -> tuple[object, str] | None:
    selected_environment = os.environ if environment is None else environment
    env = selected_environment.get(_DATA_DIR_ENV)
    if env is not None:
        if type(env) is not str:
            raise TypeError(f"{_DATA_DIR_ENV} 必须是字符串")
        if env != "":
            return env, _DATA_DIR_ENV
    from codev_platform.core.config import get, load_config

    selected_cfg = load_config() if cfg is None else cfg
    cfg_value = get(selected_cfg, "data.platform_data_dir")
    if cfg_value is not None and cfg_value != "":
        return cfg_value, "data.platform_data_dir"
    return None


def _resolved_data_path(value: object, field: str) -> Path:
    if type(value) is not str:
        raise TypeError(f"{field} 必须是字符串")
    if not value.strip() or value != value.strip():
        raise ValueError(f"{field} 不能为空或包含首尾空白")
...
def _lexical_data_path(value: object, field: str) -> Path:
    if type(value) is not str:
        raise TypeError(f"{field} 必须是字符串")
    if (
        not value.strip()
        ...
        raise ValueError(f"{field} 必须是规范绝对路径")
    portable = value.replace("\\", "/")
    if portable.startswith("//") or portable.endswith("/"):
        raise ValueError(f"{field} 必须是规范绝对路径")
    ...
    if any(not segment or segment in {".", ".."} for segment in segments):
        raise ValueError(f"{field} 必须是规范绝对路径")
    selected = Path(value)
    if not selected.is_absolute() or selected.parent == selected:
        raise ValueError(f"{field} 必须是规范绝对路径")
    return selected
...
def logs_dir() -> Path:
    ...
    日志落 data_root 才稳。本函数只定位，创建与权限收敛由写入边界负责。
    """
