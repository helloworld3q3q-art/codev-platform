...
from codev_platform.core.repo_runtime_override import (
    REINDEX_REPO_OVERRIDE_ENV as REINDEX_REPO_OVERRIDE_ENV,
    RepoOverrideEntry,
    install_runtime_override as _install_runtime_override,
    override_environment as _override_environment,
    override_from_environment as _override_from_environment,
    runtime_override as _runtime_override,
)
...
def _override_entry(spec: RepoSpec) -> RepoOverrideEntry:
    if not isinstance(spec, RepoSpec):
        raise ValueError("repo override 仓描述无效")
    return RepoOverrideEntry(
        root=spec.root,
        tag=spec.tag,
...
def _repo_spec(entry: RepoOverrideEntry) -> RepoSpec:
    return RepoSpec(
        root=entry.root,
        tag=entry.tag,
...
def install_runtime_repo_override(
    cfg: dict,
    project_id: str,
    specs: list[RepoSpec] | tuple[RepoSpec, ...],
) -> None:
    """把可信仓向量装入当前 cfg；对象键防止用户 JSON 伪造运行态值。"""
    _install_runtime_override(cfg, project_id, [_override_entry(spec) for spec in specs])


def _runtime_repo_override(cfg: dict, project_id: str) -> tuple[RepoSpec, ...] | None:
    entries = _runtime_override(cfg, project_id)
    return None if entries is None else tuple(_repo_spec(entry) for entry in entries)


def runtime_repo_override_environment(cfg: dict, project_id: str) -> dict[str, str]:
    """把进程私有仓向量编码为无秘密、限长的子进程环境载荷。"""
    return _override_environment(cfg, project_id)


def _repo_specs_from_environment(cfg: dict, project_id: str) -> tuple[RepoSpec, ...] | None:
    entries = _override_from_environment(cfg, project_id)
    return None if entries is None else tuple(_repo_spec(entry) for entry in entries)
...
def _read_meta(project_id: str) -> dict:
    """读 platform_meta/projects/<pid>/meta.json；资源缺失或损坏时优雅降级。"""
    meta_f = platform_meta_projects_dir() / project_id / "meta.json"
...
def _known_project_ids(cfg: dict) -> list[str]:
    ...
    meta_root = platform_meta_projects_dir()
    try:
...
def project_repo_specs(project_id: str, *, main_repo: Path | str | None = None,
    ...
    """
    cfg = load_config() if cfg is None else cfg
    runtime_override = _runtime_repo_override(cfg, project_id)
    if runtime_override is not None:
        return list(runtime_override)
    environment_override = _repo_specs_from_environment(cfg, project_id)
    if environment_override is not None:
        return list(environment_override)
    roots: list[Path] = []
