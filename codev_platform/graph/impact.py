...
def _resolve_duplicate_edges(edges: list[GraphEdge]) -> list[GraphEdge]:
    """冲突消解 (Phase 3): 同 (source,target,kind) 多边(多 plugin/parser 给同一关系, 或退役
    plugin 残留)→ 只保留 **provenance 更全、置信更高** 者(stamped 胜 unstamped)。保首见序,
    无冲突原样返回。
...
def build_impact_graph(conn, project_id: str, *, include_soft: bool = False,
    ...
    edges = _resolve_duplicate_edges(edges)   # 冲突消解: 同边多 plugin 重复 → 保最优 provenance
