...
def _record_failure(
    ...
    *,
    ...
    """按稳定字段去重；异常对象和正文不得进入报告。"""
    failure = IngestFailure(phase=phase, component=component, code=code)
...
def _collect_plugin_result(
    ...
    """归一单个执行结果，返回 ``(plugin, ok|failed|not_applicable)``。"""
    plugin = exec_result.plugin
    if exec_result.result is None:
        if exec_result.error_code == ERR_NOT_APPLICABLE:
            return plugin, "not_applicable"
        _record_failure(
            report,
            phase="plugin",
            component=plugin,
            code=exec_result.error_code or "UNKNOWN_FAILED",
        )
...
def _collect_merged(
    ...
    status_targets = {"ok": ok_plugins, "failed": failed_plugins}
    for repo in repos:
        for exec_result in run_all(repo, project_id):
            plugin, status = _collect_plugin_result(
                exec_result, repo, scope, report, merged, node_seen,
            )
            seen_plugins.add(plugin)
            if status in status_targets:
                status_targets[status].add(plugin)
...
def _record_frontend_source_failure(report: IngestReport) -> None:
    _record_failure(
        report,
...
def _read_frontend_usage_source(
    file: str,
    repos: list[Path],
    scope: RepoScope,
    report: IngestReport,
) -> str:
    """读取归因源码；仅记录稳定失败码，不把路径或异常正文写入报告。"""
    repo, rel = scope.resolve(file)
    candidates = [repo] if repo is not None else repos
    source = next((root / rel for root in candidates if (root / rel).is_file()), None)
    if source is None:
        _record_frontend_source_failure(report)
        return ""
    try:
...
def _frontend_api_usage_pass(store, project_id: str, report: IngestReport,
    ...
    def _read(file: str) -> str:
        return _read_frontend_usage_source(file, repos, scope, report)
...
def _set_analyzer_context(analyzer, repo_path: Path, report: IngestReport) -> None:
    """隔离单 analyzer 上下文注入，同时保留可发布门禁所需失败事实。"""
    setter = getattr(analyzer, "set_context", None)
    if not callable(setter):
        return
    try:
        setter(repo_path)
    except Exception:  # noqa: BLE001 — 单 analyzer 失败不拖垮其余
        _record_failure(
            report,
            phase="analyzer_context",
...
def _analyzers_pass(store, project_id: str, report: IngestReport,
    ...
    if repo_path is not None:
        for a in registered_analyzers():
            _set_analyzer_context(a, repo_path, report)
