...
async def list_tools() -> list[Tool]:
    return [
        ...
        Tool(
            ...
            inputSchema={
                "type": "object",
                "properties": {"limit": {"type": "integer", "description": "返回上限(默认 200)"}},
            },
        ),
        Tool(
            name="find_contract_drift",
...
def _mcp_success_payload(name: str, pid: str, payload: dict) -> dict:
    """仅在 MCP 出站边界为验收工具附加真实路由身份，保持查询引擎纯结果不变。"""
    if name != "search_nodes":
        return payload
    return {**payload, "project_id": pid, "tool": name}
...
async def call_tool(name: str, args: dict) -> list[TextContent]:
    ...
    try:
        ...
        try:
            payload = dispatch(name, args, store, pid)
            return _ok(_mcp_success_payload(name, pid, payload))
...
async def run_http(port: int = _GRAPH_SSE_PORT) -> None:
    require_managed_mcp_service("graph")
