...
_SCHEMA_PROBES = (
    f"SELECT {_NODE_COLS} FROM graph_nodes LIMIT 0",
    f"SELECT {_EDGE_COLS} FROM graph_edges LIMIT 0",
    f"SELECT {_EVIDENCE_COLS} FROM graph_evidences LIMIT 0",
    f"SELECT {_FINDING_COLS} FROM graph_findings LIMIT 0",
    "SELECT project_id, plugin, plugin_version, node_count, edge_count, evidence_count, "
    "finding_count, ingested_at FROM graph_ingest_meta LIMIT 0",
)
