...
class RepoScope:
    ...
    def __init__(self, repos: list[Path]) -> None:
        ...
        self._tag_by_repo: dict[str, str] = {
            str(repo): tag for repo, tag in zip(repos, tags, strict=True)
        }
        self._repo_by_tag: dict[str, Path] = {
            tag: repo for repo, tag in zip(repos, tags, strict=True) if tag
        }
