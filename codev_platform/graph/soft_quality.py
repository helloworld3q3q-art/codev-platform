...
def _assess_axis(nodes: list[GraphNode], edges: list[GraphEdge], *,
    ...
    成员 = 反向软边(hard --soft_edge--> soft_node)的源端硬节点。一个硬节点可属多个软节点
    (如一表跨域), 故按**边数**计成员总数(share 即"成员归属占比")。
    """
    soft_id_to_name = {n.id: n.name for n in nodes if n.kind == soft_kind}
    members_by_soft: dict[str, set[str]] = defaultdict(set)
    ...
    total = sum(len(m) for m in members_by_soft.values())
    dist = sorted(
        ({"name": soft_id_to_name[sid], "members": len(m),
          "share": (len(m) / total) if total else 0.0}
...
def _axis_flags(label: str, ax: dict, *, min_coverage: float) -> list[str]:
    ...
    flags: list[str] = []
    for g in ax["giant"]:
        flags.append(f"{label}: 巨型 cluster「{g['name']}」吃掉 {g['share']:.0%} 成员"
                     "(疑似共享节点退化, 软标签可信度存疑)")
    if ax["low_coverage"]:
...
def _render_axis(label: str, ax: dict) -> list[str]:
    ...
    lines = [
        f"## {label}",
        f"- 软节点 {ax['soft_nodes']} / 成员归属 {ax['labeled_members']} / "
        f"覆盖 {cov}(可标注 {ax['eligible']})",
    ]
    for d in ax["distribution"][:_TOP_DIST]:
        lines.append(f"    - {d['name']}: {d['members']} 成员 ({d['share']:.0%})")
