...
def serve_log_dir() -> Path:
    ...
    import codev_platform.mcp_serve as _ms
    return Path(_ms.__file__).resolve().parent / "mcp_serve_logs"
...
def run_logs(service: str, tail: int) -> int:
    ...
    sources = log_sources()
    want = list(sources) + ["serve-mcp"] + list(_JOURNAL_SERVICES) if service == "all" else [service]
