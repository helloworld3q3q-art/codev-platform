"""聚合运行观测 JSONL，并输出摘要或 Prometheus 指标。
...
只读以下由运行服务生成的观测来源：
  ...
  - chroma      : data_root/logs/search_recall.jsonl   ts/project_id/query/hit/...（每次 search_docs 一行）
  - codegraph   : data_root/logs/codegraph_usage.jsonl ts/project_id/tool/ok/...
  - reindex     : tools/chroma/reindex.log             projects/scopes fan-out blocks

纯聚合逻辑与薄 IO 层分离，测试无需触碰真实文件；本模块只读，不修改来源。
"""
...
def _agg_audit(records: list[dict]) -> dict:
    ...
    deny_reasons = Counter(str(r.get("reason")) for r in records if not r.get("allowed"))
    return {
        "total": len(records),
        "allowed": allowed,
        "denied": denied,
...
def to_prometheus(summary: MetricsSummary) -> str:
    ...
    metric(
        "codev_audit_deny_rate",
        "gauge",
        "audit deny rate in window",
    ...
    metric(
        "codev_reindex_runs_total",
        "gauge",
        "reindex runs in window",
        [("", reindex.get("total", 0))],
    )
    metric(
        "codev_reindex_fanout_runs_total",
        "gauge",
        "reindex runs targeting multiple projects",
        [("", reindex.get("fanout_runs", 0))],
    )
    metric(
        "codev_reindex_fanout_max_projects",
        "gauge",
        "max project fan-out per reindex run",
        [("", reindex.get("max_projects", 0))],
    )
    metric(
        "codev_reindex_fanout_max_scopes",
        "gauge",
        "max scope fan-out per reindex run",
...
def _source_paths() -> dict[str, Path]:
    """从稳定数据根定位 jsonl 与审计日志。"""
    from codev_platform.core.audit import audit_log_path

    return {
        "audit": audit_log_path(),
        "chroma": runtime_artifacts.chroma_recall_usage_path(),
        "codegraph": runtime_artifacts.codegraph_usage_path(),
...
def _print_human(summary: MetricsSummary, alerts: list[str], since: str | None) -> None:
    ...
    print(
        f"audit       total={a['total']}  allowed={a['allowed']}  "
    ...
    for src in ("chroma", "codegraph"):
        ...
        if "by_tool" in s:
            print(
                f"{src:<11} calls={s['total']}  errors={s['errors']}  "
    ...
    print(
        f"reindex    runs={ri['total']}  fanout_runs={ri['fanout_runs']}  "
        f"max_projects={ri['max_projects']}  max_scopes={ri['max_scopes']}"
    )
    print("-" * 48)
    print(
        f"totals  mcp_calls={summary.totals['mcp_calls']}  "
...
def run_metrics(cfg, since: str | None, as_json: bool, as_prometheus: bool = False) -> int:
...
def register(subparsers) -> None:
    mp = subparsers.add_parser("metrics", help="聚合可观测 jsonl (审计 + 三套 MCP 使用) 出指标摘要")
    mp.add_argument("--since", default=None, help="时间窗口 (如 30m / 2h / 1d / 90s; 默认全量)")
