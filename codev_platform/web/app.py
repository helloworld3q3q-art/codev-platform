"""Web Backend FastAPI 应用工厂 (plan D1/D7)。

uvicorn 以 ``factory=True`` 调 ``create_runtime_app``；模块导入阶段不解析运行身份。
中间件/异常/RequestId 全在 httpkit.build_app，本模块只列 routers + 公开 path。
"""
...
def create_runtime_app() -> FastAPI:
    """生产应用工厂：服务真正启动时解析一次运行身份，失败则拒绝启动。"""
    from codev_platform.core.runtime_identity import runtime_identity

    return create_app(identity_loader=runtime_identity)
