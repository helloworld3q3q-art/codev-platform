...
_MESSAGE_ID_TYPE = BigInteger().with_variant(Integer(), "sqlite")
...
# Agent 会话与分层记忆同属平台 PG schema；运行期 Store 只做读写和结构探针，
# 建表、补列与索引全部由 Alembic 0009 起统一管理。
agent_sessions = Table(
    "agent_sessions",
    metadata,
    Column("org_id", Text, nullable=False, server_default=text("'default'")),
    Column("user_id", Text, nullable=False),
    Column("session_id", Text, nullable=False),
    Column("project_id", Text),
    Column("created_at", DateTime(timezone=True), nullable=False, server_default=func.now()),
    Column("updated_at", DateTime(timezone=True), nullable=False, server_default=func.now()),
    PrimaryKeyConstraint("org_id", "user_id", "session_id"),
)

agent_messages = Table(
    "agent_messages",
    metadata,
    Column("id", _MESSAGE_ID_TYPE, primary_key=True, autoincrement=True),
    Column("org_id", Text, nullable=False, server_default=text("'default'")),
    Column("user_id", Text, nullable=False),
    Column("session_id", Text, nullable=False),
    Column("role", Text, nullable=False),
    Column("content", Text),
    Column("payload", _JSON_TYPE, nullable=False, server_default=text("'{}'")),
    Column("created_at", DateTime(timezone=True), nullable=False, server_default=func.now()),
)
Index(
    "ix_agent_msg_session",
    agent_messages.c.org_id,
...
memory_entries = Table(
    ...
    Column("kind", Text),
    Column("topic_key", Text),
    Column("is_redline", Boolean, nullable=False, server_default=text("false")),
    Column("status", Text, nullable=False, server_default=text("'active'")),
    Column("supersedes", Uuid(as_uuid=False), ForeignKey("memory_entries.id")),
    Column("extra", _JSON_TYPE, nullable=False, server_default=text("'{}'")),
    Column("ttl_at", DateTime(timezone=True)),
    Column("task_id", Text),
    Column("task_state", Text),
    Column("created_at", DateTime(timezone=True), nullable=False, server_default=func.now()),
    Column("updated_at", DateTime(timezone=True), nullable=False, server_default=func.now()),
)
Index(
    "ix_mem_scope",
    memory_entries.c.org_id,
    memory_entries.c.scope,
    memory_entries.c.scope_ref,
    memory_entries.c.status,
)
Index("ix_mem_topic", memory_entries.c.org_id, memory_entries.c.topic_key, memory_entries.c.status)
Index("ix_mem_task", memory_entries.c.org_id, memory_entries.c.task_id, memory_entries.c.status)

# 多机共享 reindex 队列 (PgJobQueue, alembic 0004/0007/0008)。
# per (project_id,kind) 合并(复合主键)，并登记 pending/active/result/quarantine 全量列；
...
reindex_jobs = Table(
    ...
    Column("pending_token", Text),
    Column("pending_enqueued_at", Float),
    Column("pending_updated_at", Float),
    Column("pending_meta_json", Text),
    Column("active_enqueued_at", Float),
    Column("active_meta_json", Text),
    Column("active_heartbeat_at", Float),
    Column("result_status", Text),
    Column("result_finished_at", Float),
    Column("result_enqueued_at", Float),
    Column("result_meta_json", Text),
    Column("quarantine_reason", Text),
    Column("quarantine_attempt_id", Text),
    Column("quarantine_fence", Text),
    Column("quarantine_process_identity", Text),
    Column("quarantine_containment_kind", Text),
    Column("quarantine_native_ref", Text),
    Column("quarantine_at", Float),
...
# 真值源是本 metadata 与 Alembic；PgGraphStore 运行期只做只读列探针，不再执行 DDL。
graph_nodes = Table(
    "graph_nodes",
    metadata,
