...
# 图谱可读密度上限: 返回边数 ∝ 节点数。防超大稠密项目(如 sample-project-beta 的 Java 核心)在 overview
# 里返回"发丝团": 实测 limit=2000 时 sample-project-beta 11106 边(5.6 边/节点)vs openclaw 5814(2.9)/
...
class CodegraphClient:
    ...
    def graph(self, limit: int | None, languages: list[str] | None,
        ...
        (默认 limit)绝大多数边被滤光 → 前端图全是散点无连线(sample-project-beta limit=50 实测
