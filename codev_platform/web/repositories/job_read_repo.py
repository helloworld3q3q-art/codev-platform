...
from codev_platform.web.repositories.sqlalchemy_repository import SqlAlchemyRepositoryBase
...
class PgJobReadRepo(SqlAlchemyRepositoryBase):
    """PG 读实现 —— jobs 表 select。PG 基座复用 account_store_pg._PgBase(同库/同 engine 范式,
    sqlite 单测注入 engine exercise。codex P2: job 历史持久 + 多 worker 一致。"""
