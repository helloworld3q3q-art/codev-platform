...
def report_impact(
    request: Request, body: S.ImpactRequest, ctx=Depends(require_project_access)
) -> CommonResult:
    _identity, project_id = ctx
    store = _open_store_ro(project_id)
    if store is None:
        return ok(
            S.ImpactReportResponse(found=False, summary="该项目尚无统一图谱索引"),
    ...
    return ok(
        S.ImpactReportResponse(
            ...
            impact=r.get("impact"),
            risk=r.get("risk"),
            layersAffected=r.get("layersAffected", []),
            total=r.get("total", 0),
...
def report_table_usage(
    request: Request, body: S.TableUsageRequest, ctx=Depends(require_project_access)
) -> CommonResult:
    _identity, project_id = ctx
...
def report_page_dependencies(
    request: Request, body: S.PageDepsRequest, ctx=Depends(require_project_access)
) -> CommonResult:
    _identity, project_id = ctx
...
def report_api_callers(
    request: Request, body: S.ApiCallersRequest, ctx=Depends(require_project_access)
) -> CommonResult:
    _identity, project_id = ctx
...
def report_mcp_usage(request: Request, _sess=Depends(require_platform_admin)) -> CommonResult:
    ...
    data = platform_status.mcp_usage_report()
