...
from codev_platform.reindex.target_commit import require_target_commit, resolve_project_head
...
def make_reindex_dispatch_trigger(queue_factory=None, target_commit_resolver=None) -> JobTrigger:
    ...
    queue_factory 默认 reindex.open_default_queue；target_commit_resolver 默认解析项目主仓
    当前 HEAD。两者都可在单测中注入，避免测试触及真实队列或机器级项目配置。
    """
    ...
    def _trigger(job: Job) -> None:
        ...
        resolver = target_commit_resolver or resolve_project_head
        target_commit = require_target_commit(resolver(job.project_id))
        queue = queue_factory() if queue_factory is not None else open_default_queue(fail_soft=False)
        targets = list(_kinds()) if kind == _ALL else [kind]
        meta = JobMeta(source="admin", pull_policy="never", target_commit=target_commit)
        for k in targets:
            queue.enqueue(job.project_id, k, meta=meta)
