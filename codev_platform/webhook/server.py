...
def webhook_port(cfg: dict | None = None) -> int:
    return int(
        _cfg_get(cfg if cfg is not None else load_config(), "webhook.port") or DEFAULT_WEBHOOK_PORT
    )


def build_app(
    middleware=None,
    status_authenticator=None,
    identity_loader: Callable[[], RuntimeIdentity] | None = None,
):
    ...
    from codev_platform.gateway import Unauthorized, build_authenticator

    if identity_loader is None:
        from codev_platform.core.runtime_identity import runtime_identity

        identity_loader = runtime_identity
    runtime = identity_loader()
    status_auth = (
        status_authenticator
        if status_authenticator is not None
        else build_authenticator(load_config())
    )
    ...
    async def handle(request):
        ...
        if prov is None:
            return JSONResponse(
                {"error": f"unknown provider '{name}'", "code": ErrorCode.INVALID_PARAMS.value},
                status_code=404,
            )
        ...
        if clen is not None:
            try:
                if int(clen) > _MAX_BODY:
                    ...
                    return JSONResponse(
                        {"error": "payload too large", "code": ErrorCode.INVALID_PARAMS.value},
                        status_code=413,
                    )
            except ValueError:
                return JSONResponse(
                    {"error": "bad content-length", "code": ErrorCode.INVALID_PARAMS.value},
                    status_code=400,
                )
        ...
        if not secret:
            if not allow_insecure:
                _log(
                    f"[{name}] webhook.secret 未配置, fail-closed 拒绝; 本机信任可设 webhook.allow_insecure=true"
                )
                return JSONResponse(
                    {
                        "error": "webhook secret not configured",
                        "code": ErrorCode.DEPENDENCY_MISSING.value,
                    },
                    status_code=401,
                )
            _log(
                f"[{name}] webhook.secret 未配置, 但 webhook.allow_insecure=true, 跳过验签放行 (仅限本机信任)"
            )
        elif not prov.verify(request.headers, body, secret):
            _log(f"[{name}] 验签失败 (检查 webhook.secret 与 VCS 配置一致)")
            return JSONResponse(
                {"error": "invalid signature", "code": ErrorCode.ACCESS_DENIED.value},
                status_code=401,
            )
        try:
            ...
            return JSONResponse(
                {"error": "bad json", "code": ErrorCode.INVALID_PARAMS.value}, status_code=400
            )
        ...
        try:
            target_commit = require_target_commit(event.target_commit)
        except TargetCommitError:
            _log(f"[{name}] push 事件目标提交无效, 拒绝入队")
            return JSONResponse(
                {"error": "invalid target commit", "code": ErrorCode.INVALID_PARAMS.value},
                status_code=400,
            )
        ...
        if not plan:
            _log(
                f"[{name}] {event.repo} -> {pid}: {len(event.changed_files)} 文件改动但无 reindex scope 命中, 跳过"
            )
            return JSONResponse(
                {
                    "ok": True,
                    "project_id": pid,
                    "projects": _target_projects_for(cfg, pid),
        ...
        _log(
            f"[{name}] {event.repo} -> {summary} enqueue ({len(event.changed_files)} files changed)"
        )
        return JSONResponse(
            {
                "ok": True,
    ...
    async def platform_status(request):
        # webhook POST 仍只走 provider HMAC；运行状态单独复用 gateway 认证策略。
        try:
            status_auth.authenticate(request.headers, request.url.query)
        except Unauthorized:
...
async def run_http(port: int | None = None) -> None:
    ...
    from codev_platform.webhook import providers
    from codev_platform.gateway import build_authenticator, maybe_rate_limit_middleware

    cfg = load_config()
    if port is None:
        port = webhook_port(cfg)
    for issue in webhook_mapping_warnings(cfg):
        _log(f"[startup] {issue}")
    # webhook 无身份 (无 Auth 中间件) → 限流按 client IP (default_key_from_scope 自动回退); dev 默认关
    _mw = []
    _rl = maybe_rate_limit_middleware(cfg)
    if _rl is not None:
        _mw.append(_rl)
    app = build_app(middleware=_mw, status_authenticator=build_authenticator(cfg))
    _log(
        f"[http] webhook receiver starting on 127.0.0.1:{port} (providers: {', '.join(providers.names())})"
    )
...
def webhook_mapping_warnings(cfg: dict | None = None) -> list[str]:
    ...
    from codev_platform.core.repos import (
        webhook_enabled_project_ids,
        webhook_extra_repo_mapping_issues,
    )

    cfg = load_config() if cfg is None else cfg
    return [
        f"extra repo webhook 未映射: project={i['project_id']} extra={i['extra']} reason={i['reason']}"
        for i in webhook_extra_repo_mapping_issues(
            cfg, project_ids=webhook_enabled_project_ids(cfg)
        )
    ]
