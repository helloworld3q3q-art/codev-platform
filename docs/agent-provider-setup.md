...
配置文件:**WSL 的 `~/.codev-platform/config.json`**(= `/home/user/project`,**用户级、不进 git**)。结构已由我预置好(4 家 provider,api_key 全空),你只需补**你用的那一家**。
...
"agent": {
  ...
  "providers": {
    "deepseek": { "base_url": "https://example.invalid/reference", "model": "deepseek-chat",
...
本次体检发现 **WSL config 无 provider key**(干净)。审计 #7 那个明文 key 多半在 **Windows** 的 `C:\workspace\project`。查一下:
```powershell
type C:\workspace\project
```
