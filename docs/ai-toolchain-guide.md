...
基于 Chroma (https://example.invalid/reference) 向量数据库 + 本地嵌入模型,把所有 markdown 文档切成"块 (chunk)" → 向量化 → 存到本地 SQLite。
...
```text
<PLATFORM_DATA_DIR>/logs/search_recall.jsonl
...
| Chroma | `<PLATFORM_DATA_DIR>/logs/search_recall.jsonl` | 召回日志 |
...
| MEMORY | `C:\workspace\project` | 跨会话用户记忆 |
