...
> **背景**：codev-platform 名义是平台真值源,但运行时重资产(venv 4.66GB / 模型 launcher / 数据库 / data 目录)物理住在 `C:\workspace\project`(= openclaw-stock 业务仓)。这是历史包袱 —— 平台运维寄生在业务仓里。
>
...
pip install torch==2.11.0+cu128 --index-url https://example.invalid/reference
...
apply(P2+P4 全绿后):
```powershell
Move-Item C:\workspace\project C:\workspace\project -Force
Move-Item C:\workspace\project C:\workspace\project -Force
Move-Item C:\workspace\project C:\workspace\project -Force
```
