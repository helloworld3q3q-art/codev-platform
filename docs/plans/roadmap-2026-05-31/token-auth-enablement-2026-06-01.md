...
- **passthrough 绑非 loopback**：启动期 `auth.py` 会打 WARN（未认证对外开放）。生产严禁 passthrough 监听非 internal.example.invalid。
- **跨机必须配 TLS**：token 走 HTTP header，裸 HTTP 跨机即明文裸奔。跨机部署须前置反代终止 TLS，业务仓 `.mcp.json` 切 `https://`，服务仅监听 internal.example.invalid —— 详见 `multi-org-server-hardening-2026-05-31.md §四 P3 A3`。
