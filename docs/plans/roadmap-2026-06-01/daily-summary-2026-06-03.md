...
- 后端实际跑在 WSL `/home/user/project`;前端 proxy/api/enums 指向 WSL IP `internal.example.invalid:18088`。
...
**提交区间** `b67d7f2 … 2943742`(13 commit;runner-timeout 在并发会话 `64b0dae`)。全推 Gitea + WSL `/home/user/project` ff-pull 同步 + codegraph reindex,MCP 索引验证含 `resolve_membership`/`SessionAwareAuthenticator` 新符号。
