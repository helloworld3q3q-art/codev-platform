...
| 项 | 默认 | 远程部署要求 |
|---|---|---|
| 监听地址 | 所有服务绑 `internal.example.invalid` | 暴露公网必须走反代 + TLS, 不直接改 host=0.0.0.0 |
...
pip install -e C:\workspace\project           # 需 agent HTTP 面再加 [agent]
...
- [ ] 服务仍绑 `internal.example.invalid`, 公网仅经反代 (nginx/caddy) + TLS 暴露。
