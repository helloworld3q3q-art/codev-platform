...
## 续-commit 链
`a394fa8`/`e73dbdc`/`39218b1`/`4721e82`(config DI)→ `ca46c35`(soft-quality+regen)→ `fae0b1e`(gitattributes)→ `0f52e0d`(set_roles 多 org)→ `d4984e5`(审计优化)→ `fac6aae`(#8 后续)→ `31f40f8`/`185c60a`(多仓 plan)→ `72f357c`/`58b74a6`(#1 switch-org)→ `4f74020`/`58d7347`(#3+#5)→ `d91fb81`/`0dc135a`(前端所属组织列 + 切换 reload)。全部 push `origin/dev` + WSL 同步。
...
## 续2-commit 链
`5956fa9`(recall 6→24 + 跨项目 + project_id 过滤 + 防毒丸守卫)→ `37ce633`(delta paired bootstrap CI + `_bootstrap_ci` 抽 `_common`)。push `origin/dev` → WSL `origin` ff,两项目真跑出 CI,WSL 8 passed。
