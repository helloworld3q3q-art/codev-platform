...
> commit 链:`76d5ea8`(P1)→`ac0126d`(P4)→`379f42c`(P2 token_issue 基础)。全程 fuwuqi/dev,相关单测全绿。
...
## commit 链(产品化迭代段)
`76d5ea8`(P1 onboard 补 sync+.mcp.json + 抽公共核心)→`71af7f8`(P1 plan 标交付)→`ac0126d`(P4 接入指南重写 + 治理看板已存在/能力矩阵伪需求)→`379f42c`(P2 token_issue 共享 + CLI 去重)。
