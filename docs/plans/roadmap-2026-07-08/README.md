...
| 文件 | 内容 | 状态 |
|---|---|---|
| [reindex-worker-lifecycle-plan-2026-07-08.md](reindex-worker-lifecycle-plan-2026-07-08.md) | reindex 队列自动拉起短驻 worker、idle 退出、worker 健康状态、drain-once 运维入口 | 🟢 P1-P6 已落地;WSL 服务/token 固化已审计;runner log 演练与收尾审计已完成 |
...
- roadmap 收尾审计:已关闭项包括 Windows 短驻 worker、WSL systemd 常驻 worker、WSL/服务器 env-file 注入能力、WSL 本机 token 落地、`health --all` env-file 兜底、runner log 脱敏/大小上限/失败 note 演练。未关闭项拆为独立后续任务:WSL `openclaw-stock/chroma` 的 Chroma `disk I/O error` 运行态排查;生产服务器首次落地 `/etc/codev-platform/platform.env`、systemd 安装和 `health --all` 实调;如需在生产演练 runner failure,使用独立 `PLATFORM_DATA_DIR` 或明确 drill project 前缀。
...
- 收尾审计实调:WSL `codev-platform` 已拉到 `4774c9a`,systemd worker 完成本次文档 chroma reindex,队列空,`health --mode light` READY。WSL `openclaw-stock/chroma` manifest 当前 `status=failed`,note 长度 `1215`,含 `disk I/O error`;`code_vec/codegraph/ingest` 均为 `ok` 且对齐 HEAD,因此拆为单独运行态修复任务。
