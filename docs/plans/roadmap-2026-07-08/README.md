...
- `hook missed?` manifest 兜底经测试审计兄弟二次审查后收窄为 `chroma` only,补齐 `failed`/`stale`/`unknown`/unreadable/project unknown 分支和 legacy log 优先级覆盖;目标回归 `16 passed` + 邻近 health/manifest/CLI `33 passed`(合计 `49 passed`),ruff 目标文件通过,Windows health READY。
- WSL 真实 repo/data 加载新代码验证:docs-only HEAD 可由 `chroma` manifest 兜底为 READY;本次代码变更 HEAD 会保守 WARN `manifest fallback limited to chroma-only`,队列已空且 worker 正常,后续需补 codegraph/ingest/code_vec 强成功信号或独立 artifact proof 后再取消该 WARN。
