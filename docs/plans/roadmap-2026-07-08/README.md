...
> 2026-07-13 口径更新：本文中用户目录 `EnvironmentFile` 与 `EnvironmentFile=-...` 仅为历史记录，不能用于当前 WSL 发布或受控 CodeGraph 恢复。新部署必须使用 root 受控、必需的 `/etc/codev-platform/*.env`，详见 2026-07-13 隔离维护运行手册。

> **当前主目录**:reindex worker 短驻化 + 队列可观测收尾。
>
...
| 文件 | 内容 | 状态 |
|---|---|---|
| [reindex-worker-lifecycle-plan-2026-07-08.md](reindex-worker-lifecycle-plan-2026-07-08.md) | reindex 队列自动拉起短驻 worker、idle 退出、worker 健康状态、drain-once 运维入口 | 🟢 P1-P6 已落地;WSL 服务/token 固化、runner log 演练、openclaw Chroma 排查均已关闭 |
...
- WSL 服务固化审计确认 `/home/user/project` 已在 `0f6d61c`,`codev-reindex.service` 已安装到 `/etc/systemd/system/` 且 `enabled` + `active running`;单元内容走仓内 `mcp_systemd.render_reindex_unit()` 的正式路线:`ExecStart=/home/user/project -m codev_platform.cli reindex-queue worker`,`Restart=always`,`RestartSec=3`。未发现额外 nohup/手动 worker 残留,`~/codev-systemd/` 仅作为生成 unit/install.sh 的重装来源。WSL 服务固化本步不新增 supervisor/nohup,不手改运行态 unit。
...
- WSL token 环境注入落地:WSL 本机使用 `/home/user/project` 保存机器私有 token 环境变量,目录权限 `0700`,env 文件和 `~/.codev-platform/config.json` 及历史备份均收紧到 `0600`;用户级 config 设置 `platform.token_env=CODEV_PLATFORM_MCP_TOKEN` 和 `systemd.env_file=<env file>`。重新生成并安装 systemd unit 后,7 个常驻服务进程均确认带有 `CODEV_PLATFORM_MCP_TOKEN` 环境变量;`health --all` 客户端新增从 `systemd.env_file` 读取 token 的兜底,避免交互 shell 必须手工 `source`。
- runner log 真实故障演练:Windows 本机用临时非 git/非项目 repo 触发 `reindex --chroma --repo <temp>` 返回 rc=1,经真实 `CliReindexRunner`/`ReindexWorker` 生成 `data/logs/reindex-runner/runner-log-drill__chroma.log` 和 `runner-log-drill/chroma` failed manifest note;该演练不写真实 `chroma/codegraph/ingest/code_vec` 产物。无效 CLI 参数返回 rc=2,会被 worker 视为锁占用/db busy retry,不会写 failed manifest,因此仅记录为不适合的失败演练样本。测试审计兄弟复核通过,同时提醒该演练会在共享 data root 留下合成 `runner-log-drill` 记录;服务器上应使用明确 drill 前缀或独立 `PLATFORM_DATA_DIR`。
- roadmap 收尾审计:已关闭项包括 Windows 短驻 worker、WSL systemd 常驻 worker、WSL/服务器 env-file 注入能力、WSL 本机 token 落地、`health --all` env-file 兜底、runner log 脱敏/大小上限/失败 note 演练。后续又关闭 WSL `openclaw-stock/chroma` 运行态排查:通过正式 CLI blue-green force rebuild,将 openclaw Chroma current build 对齐到 4914 chunks,`index_manifest.sqlite` 四类全 `ok`,WSL MCP `search_docs` 可检索。剩余后续任务:生产服务器首次落地 `/etc/codev-platform/platform.env`、systemd 安装和 `health --all` 实调;如需在生产演练 runner failure,使用独立 `PLATFORM_DATA_DIR` 或明确 drill project 前缀。
...
- 历史状态:Windows 本机 `reindex-queue status` 显示 FileSpoolQueue 队列空,短驻 worker 已 idle 退出;WSL `codev-reindex.service` 为 systemd 常驻 `mode=forever`,队列空,health READY。当时剩余的 WSL token 环境注入风险已关闭。
...
- `health --all` env-file 兜底验证:先用 TDD 写 `test_health_all_reads_token_from_configured_systemd_env_file`,确认未实现时无 Authorization;审计后补齐“任一进程 env 优先于 env-file”和“env-file shell-like 值按字面处理”两个回归。目标回归:`python -m pytest tests/test_health_all_auth.py tests/test_health_split_security.py -q` → `13 passed / 1 warning`;扩大回归 `tests/test_mcp_serve.py tests/test_systemd_restart.py tests/test_systemd_agent_clock.py tests/test_health_all_auth.py tests/test_health_split_security.py` → `57 passed / 1 warning`;`python -m ruff check codev_platform/ops/health/__init__.py tests/test_health_all_auth.py` → passed。
- WSL token 环境注入实调:推送 `59e11de` 后,WSL 自动拉取并由 systemd `codev-reindex.service` 完成 `codev-platform` 的 `chroma/codegraph/ingest/code_vec`;队列空,`health --mode light` READY。显式清掉交互 shell 的 `PLATFORM_TOKEN` / `CODEV_PLATFORM_MCP_TOKEN` 后运行 `health --all` 仍成功,证明客户端可从 `systemd.env_file` 读取 token。WSL 配置检查:`platform.token_env=CODEV_PLATFORM_MCP_TOKEN`,`systemd.env_file=/home/user/project`,env 文件权限 `0600`,安装 unit 中共有 8 处 `EnvironmentFile=-/home/helloworld/.config/codev-platform/platform.env`。
- runner log 真实故障演练审计:Windows `data/logs/reindex-runner/*.log` 共 13 个,默认上限 `524288` bytes,`oversized=[]`,用 `redact_runner_output()` 复扫 `unredacted_sensitive_shapes=[]`;测试审计兄弟额外扫 JWT/GitHub PAT/AWS/Google/Slack/裸长高熵串,结果 0 命中。演练日志 `runner-log-drill__chroma.log` 大小 `696` bytes,含 `FAIL: chroma reindex exit=1`;manifest `runner-log-drill/chroma` 为 `status=failed`,note 长度 `696`,可读并包含失败原因。WSL `data/logs/reindex-runner/*.log` 共 8 个,同样 `oversized=[]` 且 `unredacted_sensitive_shapes=[]`;`codev-platform__code_vec.log` 含 `proof: code_vec ok`。WSL 同期出现 `openclaw-stock__chroma.log` 真实 `disk I/O error` 失败日志,本轮记为 openclaw 独立运行态风险,不阻塞 codev-platform token/runner-log 验收。
- openclaw Chroma 运行态收尾:WSL ext4/空间/inode 正常,`dmesg` 无真实磁盘 I/O 错误,旧 `chroma.sqlite3` 只读 `integrity_check/quick_check` 为 `ok`;通过 `PLATFORM_INDEX_FLUSH_BATCH=100000 codev-platform reindex --repo /home/user/project --chroma --force` 对 openclaw HEAD `5335c969` 做 latest force rebuild。最终 current build `5335c9693c7c-1783572706`,fresh Chroma count=4914,build manifest=267 files / 4914 chunks,`index_manifest.sqlite` 中 `chroma/codegraph/ingest/code_vec` 全 `ok`,队列空,`health --all` 显示 openclaw-stock Chroma=4914 chunks,WSL MCP `search_docs` 实调可检索。
