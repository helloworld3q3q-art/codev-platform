...
- WSL token 环境注入实调:推送 `59e11de` 后,WSL 自动拉取并由 systemd `codev-reindex.service` 完成 `codev-platform` 的 `chroma/codegraph/ingest/code_vec`;队列空,`health --mode light` READY。显式清掉交互 shell 的 `PLATFORM_TOKEN` / `CODEV_PLATFORM_MCP_TOKEN` 后运行 `health --all` 仍成功,证明客户端可从 `systemd.env_file` 读取 token。WSL 配置检查:`platform.token_env=CODEV_PLATFORM_MCP_TOKEN`,`systemd.env_file=/home/helloworld/.config/codev-platform/platform.env`,env 文件权限 `0600`,安装 unit 中共有 8 处 `EnvironmentFile=-/home/helloworld/.config/codev-platform/platform.env`。
...
- openclaw Chroma 运行态收尾:WSL ext4/空间/inode 正常,`dmesg` 无真实磁盘 I/O 错误,旧 `chroma.sqlite3` 只读 `integrity_check/quick_check` 为 `ok`;通过 `PLATFORM_INDEX_FLUSH_BATCH=100000 codev-platform reindex --repo /home/helloworld/work/platform --chroma --force` 对 openclaw HEAD `5335c969` 做 latest force rebuild。最终 current build `5335c9693c7c-1783572706`,fresh Chroma count=4914,build manifest=267 files / 4914 chunks,`index_manifest.sqlite` 中 `chroma/codegraph/ingest/code_vec` 全 `ok`,队列空,`health --all` 显示 openclaw-stock Chroma=4914 chunks,WSL MCP `search_docs` 实调可检索。
