...
- `hook missed?` 诊断改为复用 reindex scope 公共推导,并在 worker/manifest 模式下仅对 docs 触发的 `chroma` 兜底判绿;测试审计指出 codegraph/ingest/code_vec runner 仍可能 fail-soft,因此代码链路不单靠 manifest 判绿。目标回归: `python -m pytest tests/test_health_hook_missed.py tests/test_post_hooks.py` → `17 passed`;`python -m pytest tests/test_health_reindex_worker.py tests/test_index_manifest.py tests/test_wait_for_reindex_worker.py tests/test_health_webhook_mapping.py tests/test_health_all_auth.py tests/test_cli_parser.py` → `33 passed`;合计目标回归 `50 passed`;`python -m ruff check codev_platform/ops/health/__init__.py codev_platform/ops/health/_checks.py codev_platform/ops/reindex/__init__.py codev_platform/ops/reindex/dispatch.py tests/test_health_hook_missed.py tests/test_post_hooks.py` → passed;Windows health READY。WSL 真实 repo/data 验证:docs-only HEAD 可由 `chroma` manifest 兜底为 READY;本次代码变更 HEAD 已消费完队列,但 `codev-platform__chroma` job rc=1,health 保守 WARN;后续需补 worker runner 输出/失败定位和 codegraph/ingest/code_vec 强成功信号或独立 artifact proof。
