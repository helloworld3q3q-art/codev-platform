...
- WSL 实际单元:`/etc/systemd/system/codev-reindex.service`,`User=helloworld`,`WorkingDirectory=/home/user/project`,`ExecStart=/home/user/project -m codev_platform.cli reindex-queue worker`,`Restart=always`,`RestartSec=3`。
...
> 2026-07-13 口径更新：本节保留历史实施记录，但其中用户目录下的 `EnvironmentFile` 示例已废弃。当前 Linux/WSL systemd 环境文件必须迁至 root 受控的 `/etc/codev-platform/*.env`，文件及父目录链不可被组/其他用户写入；渲染器不再生成可忽略缺失的 `EnvironmentFile=-...`。未迁移的历史路径不能通过受控 CodeGraph 恢复的同源证明。
...
- WSL 使用 `/home/user/project` 作为机器私有 env 文件;目录权限 `0700`,env 文件权限 `0600`。
- `~/.codev-platform/config.json` 设置 `platform.token_env=CODEV_PLATFORM_MCP_TOKEN` 和 `systemd.env_file=/home/user/project`;当前 config 与历史 `config.json.bak*` 权限均为 `0600`。
...
- WSL 配置确认:`platform.token_env=CODEV_PLATFORM_MCP_TOKEN`,`systemd.env_file=/home/user/project`;env 文件存在且权限 `0600`;安装后的 systemd unit 中共有 8 处 `EnvironmentFile=-/home/helloworld/.config/codev-platform/platform.env`。
