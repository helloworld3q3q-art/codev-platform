...
- `hook missed?` 诊断改为复用 reindex scope 公共推导,并在 worker/manifest 模式下仅对 docs 触发的 `chroma` 兜底判绿;测试审计指出 codegraph/ingest/code_vec runner 仍可能 fail-soft,因此代码链路不单靠 manifest 判绿。目标回归: `python -m pytest tests/test_health_hook_missed.py tests/test_post_hooks.py` → `15 passed`;`python -m pytest tests/test_health_reindex_worker.py tests/test_index_manifest.py tests/test_wait_for_reindex_worker.py tests/test_health_webhook_mapping.py tests/test_health_all_auth.py tests/test_cli_parser.py` → `33 passed`;`python -m ruff check codev_platform/ops/health/__init__.py codev_platform/ops/health/_checks.py codev_platform/ops/reindex/__init__.py codev_platform/ops/reindex/dispatch.py tests/test_health_hook_missed.py tests/test_post_hooks.py` → passed;Windows health READY,WSL 真实 repo/data 加载新代码验证 READY。
