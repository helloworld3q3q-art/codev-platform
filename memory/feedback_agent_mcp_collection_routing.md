...
1. **指定 cwd**:`cwd: C:\workspace\project` — agent Bash/PowerShell 调用 daemon 时通过 cwd 推 project_id
...
- 审计 codev-platform 仓 agent:`prompt 加 "cd C:\workspace\project 后调 search_docs,daemon 会按 .claude/project.json load codev-platform__platform_docs collection"`
