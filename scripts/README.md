...
前提:本机已有 PostgreSQL 在跑(如 internal.example.invalid:5432)。若无原生 PG,改用仓根 `docker-compose.yml`(端口 5433),跳过步骤 1。
...
2. **配 config.memory.pg_dsn**(把上一步设的密码原样填进去):
   ...
   "memory": { "pg_dsn": "postgresql://example.invalid/database:<你的密码>@internal.example.invalid:5432/codev_platform_memory" }
   ```
   更安全的做法:DSN 走 env,不进 config 文件 —— `setx CODEV_PLATFORM_MEMORY_DSN "postgresql://example.invalid/database:<密码>@internal.example.invalid:5432/codev_platform_memory"`(env 优先于 config)。docker 方案端口改 `5433`。
...
- 连库通(会提示输入步骤 1 设的密码):
  ```
  psql -h internal.example.invalid -U codev_platform -d codev_platform_memory -c "SELECT current_database(), current_user;"
  ```
- 表自动建出(agent serve 起来并发起过一次会话后):
  ```
  psql -h internal.example.invalid -U codev_platform -d codev_platform_memory -c "\dt"
  ```
