...
function Resolve-PlatformDataPath {
    param([string]$Value)
    if ([string]::IsNullOrWhiteSpace($Value) -or $Value -ne $Value.Trim()) {
        throw 'PLATFORM_DATA_DIR must not be empty or contain surrounding whitespace'
    }
    if (-not [System.IO.Path]::IsPathRooted($Value)) {
        throw 'PLATFORM_DATA_DIR must be an absolute path'
    }
...
# so its parent is the codev-platform root.
...
# NOT in the audited -Repo. Runtime data resolution mirrors core.paths.data_root:
# environment override, then user config, then codev-platform\data.
$PlatformDataDefault = Get-CfgPath 'data.platform_data_dir'
if ($env:PLATFORM_DATA_DIR) {
    $PlatformDataDir = Resolve-PlatformDataPath $env:PLATFORM_DATA_DIR
} elseif ($PlatformDataDefault) {
    $PlatformDataDir = Resolve-PlatformDataPath $PlatformDataDefault
} else {
    $PlatformDataDir = Join-Path $CodevRoot 'data'
}
$ChromaDir  = Join-Path $CodevRoot 'codev_platform\chroma'
$ChromaPy   = Join-Path $CodevRoot '.venv\Scripts\python.exe'
$ChromaData = Join-Path $PlatformDataDir 'chroma'
...
# The shared recall log follows the same stable data root as the Python services.
$recallFile = Join-Path $PlatformDataDir 'logs\search_recall.jsonl'
try {
