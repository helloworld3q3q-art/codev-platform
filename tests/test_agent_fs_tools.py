...
def multirepo(tmp_path, monkeypatch):
    main = tmp_path / "main"
    (main / "ideas-private").mkdir(parents=True)
    (main / "ideas-private" / "Ctrl.java").write_text("class Ctrl {}\n", encoding="utf-8")
    pda = tmp_path / "pda"
    (pda / "pages" / "pick").mkdir(parents=True)
    ...
    monkeypatch.setattr(fs, "resolve_project_id", lambda explicit: "sample-project-beta")
    return main, pda


def test_read_file_finds_in_extra_repo(multirepo):
    # 文件只在 extra_repo(PDA 仓)→ 仍能读到(关联仓可读)。
    r = fs.ReadFileTool("sample-project-beta").run({"path": "pages/pick/scan.vue"})
...
def test_list_dir_subpath_in_extra_repo(multirepo):
    r = fs.ListDirTool("sample-project-beta").run({"path": "pages/pick"})
    assert not r.is_error and "f scan.vue" in r.content


def test_list_dir_resolves_tagged_extra_path(multirepo):
    r = fs.ListDirTool("sample-project-beta").run({"path": "pda::pages/pick"})
    assert not r.is_error and "f scan.vue" in r.content


def test_multirepo_traversal_still_blocked(multirepo):
    r = fs.ReadFileTool("sample-project-beta").run({"path": "../../../etc/passwd"})
    assert r.is_error and "穿越" in r.content
