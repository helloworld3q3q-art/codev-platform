...
def test_audit_log_path_under_data_root(monkeypatch, tmp_path):
    selected = tmp_path / "尚未创建"
    monkeypatch.setattr(audit, "data_root", lambda: selected)
    p = audit.audit_log_path()
    assert p == selected / "audit" / "access.jsonl"
    assert not selected.exists()


def test_audit_access使用私有权限写入(tmp_path, monkeypatch):
    path = tmp_path / "audit" / "access.jsonl"
    monkeypatch.setattr(audit, "audit_log_path", lambda: path)

    audit.audit_access("graph", _ident(), "demo", _dec(False, "denied"))
