...
def test_build_text_caps_oversized_docstring_and_signature():
    # 实测 sample-project-beta 有 docstring 达 17KB 的 Java 方法 → base 撑爆每个 chunk(慢 encode + 稀释)。
...
def test_collect_node_chunks_skips_missing_extra_but_not_main(tmp_path, monkeypatch, caplog):
    from codev_platform.core.repos import RepoSpec
    main = tmp_path / "main"
    main.mkdir()
    extra = tmp_path / "extra"
    ...
    assert str(extra.resolve()) not in str(captured.value)
    assert str(extra.resolve()) not in caplog.text


# ---- 增量续跑前探活(被中断写坏的库 → 退全量自愈) ----

def test_existing_chroma_healthy_requires_sqlite_file(tmp_path):
    """checkpoint 缺少 SQLite 不能证明可恢复，必须跳过。"""
