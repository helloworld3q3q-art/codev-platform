...
def test_analyze_error_is_fail_soft(clean_registry, tmp_path):
    ...
    try:
        ...
        _analyzers_pass(conn, "p", report)  # 不抛
        g = conn.load_graph("p", plugin=ANALYZERS_PLUGIN)
        # 崩的被跳过, 正常 analyzer 的软产物仍落库。
        assert any(n.kind == NodeKind.BUSINESS_DOMAIN.value for n in g.nodes)
        assert IngestFailure(
            phase="analyzer",
            component="boom",
            code="ANALYZE_FAILED",
        ) in report.failures
        assert "analyze boom" not in repr(report.failures)
    finally:
        conn.close()


def test_analyzer_context_error_is_structured_failure(clean_registry, tmp_path):
    from codev_platform.graph.ingest import IngestFailure, IngestReport, _analyzers_pass
    from codev_platform.graph.store import open_store

    class _ContextBoom:
        name = "context_boom"

        def set_context(self, _repo_path):
            raise RuntimeError("context secret")

        def applies(self, _nodes):
            return False

        def analyze(self, _project_id, _nodes, _edges):  # pragma: no cover
            raise AssertionError("不适用 analyzer 不应执行")

    register_analyzer(_ContextBoom())
    conn = open_store("p", path=tmp_path / "g.sqlite")
    try:
        report = IngestReport(project_id="p")
        _analyzers_pass(conn, "p", report, tmp_path)
        assert report.failures == [
            IngestFailure(
                phase="analyzer_context",
                component="context_boom",
                code="CONTEXT_FAILED",
            )
        ]
        assert "context secret" not in repr(report.failures)
    finally:
        conn.close()
