...
def test_health_all_connection_failure_explains_service_recovery_not_reindex(monkeypatch, capsys):
    _patch_config(monkeypatch, {"platform": {"url": "http://127.0.0.1:18083"}})
    ...
    assert health_mod.cmd_health_all(object()) == 1
    captured = capsys.readouterr().out
    assert "serve-mcp start" in captured
    assert "serve-mcp status" in captured
    assert "不代表索引" in captured
    assert "reindex" not in captured


def test_health_all_uses_platform_source_for_windows_wsl_data_owner(monkeypatch):
    cfg = {
        "data": {
            "platform_data_dir": r"\\wsl.localhost\Ubuntu\home\demo\data",
        },
        "mcp": {"platform_docs_sse_port": 18083},
    }
    monkeypatch.setattr(
        health_mod,
        "wsl_data_owner",
    ...
    assert health_mod._platform_url(cfg) == "http://127.0.0.1:19083/platform/status"
