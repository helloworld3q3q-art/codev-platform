...
# ---------------------------------------------------------------------------
# webhook: 真 build_app（状态面独立认证；默认测试配置为 passthrough）
# ---------------------------------------------------------------------------
def test_webhook_healthz_minimal_no_provider_leak():
    from codev_platform.webhook.server import build_app

    client = TestClient(build_app(identity_loader=fake_runtime_identity))
    r = client.get("/healthz")
    assert r.status_code == 200
    body = r.json()
    ...
    # provider 清单只在 /platform/status；passthrough 模式下独立认证器放行。
