...
def test_log_sources只读定位不创建数据目录(tmp_path, monkeypatch):
...
def test_serve_log_dir_named_mcp_serve_logs且位于数据根(tmp_path, monkeypatch):
    monkeypatch.setenv("PLATFORM_DATA_DIR", str(tmp_path))

    assert L.serve_log_dir() == tmp_path / "mcp_serve_logs"
