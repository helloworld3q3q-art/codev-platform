...
def test_指标读取端与写入端共享数据根运行文件真值(tmp_path, monkeypatch):
    monkeypatch.setenv("PLATFORM_DATA_DIR", str(tmp_path))

    sources = m._source_paths()

    assert sources["chroma"] == tmp_path / "logs" / "search_recall.jsonl"
    assert sources["codegraph"] == tmp_path / "logs" / "codegraph_usage.jsonl"
